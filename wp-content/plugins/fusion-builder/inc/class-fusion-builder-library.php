...
class Fusion_Builder_Library {
	...
	private function __construct() {
		...
		add_action( 'wp_ajax_fusion_builder_load_demo_header', [ $this, 'load_demo_header' ] );
	 ...
	 */
	public function load_demo_header() {
		check_ajax_referer( 'fusion_load_nonce', 'fusion_load_nonce' );
		$data                        = [];
		$fusion_builder_demo_headers = apply_filters( 'fusion_builder_get_demo_headers', [] );

		if ( isset( $_POST['header_name'] ) && '' !== $_POST['header_name'] && isset( $_POST['post_id'] ) && '' !== $_POST['post_id'] ) {
			$header_name = sanitize_text_field( wp_unslash( $_POST['header_name'] ) );
			$post_id     = (int) $_POST['post_id'];

			if ( isset( $fusion_builder_demo_headers[ $header_name ] ) ) {
				$data['post_content'] = $fusion_builder_demo_headers[ $header_name ]['content'];
				wp_send_json_success( $data );
			}
		}
		wp_send_json_error( $fusion_builder_demo_headers, 500 );
	}
		...
		<div class="fusion_builder_modal_settings">
			<div class="fusion-builder-modal-top-container">
				...
				<ul class="fusion-tabs-menu">
					...
					<?php endif; ?>
					<li><a href="#fusion-builder-layouts-templates" id="fusion-builder-layouts-templates-trigger"><?php esc_html_e( 'Templates', 'fusion-builder' ); ?></a></li>
					<li><a href="#fusion-builder-layouts-sections" id="fusion-builder-layouts-sections-trigger"><?php esc_html_e( 'Containers', 'fusion-builder' ); ?></a></li>
					<li><a href="#fusion-builder-layouts-columns" id="fusion-builder-layouts-columns-trigger"><?php esc_html_e( 'Columns', 'fusion-builder' ); ?></a></li>
					<li><a href="#fusion-builder-layouts-elements" id="fusion-builder-layouts-elements-trigger"><?php esc_html_e( 'Elements', 'fusion-builder' ); ?></a></li>
				</ul>
			</div>
								...
								<?php foreach ( $fusion_builder_demo_headers as $header_key => $header ) : ?>
									<li class="fusion-page-layout">
										<?php if ( isset( $header['image'] ) ) : ?>
											...
											?>
											<a href="#" class="fusion-builder-demo-header-button-load" data-header-name="<?php echo esc_attr( $header_key ); ?>" data-post-id="<?php echo esc_attr( get_the_ID() ); ?>">
